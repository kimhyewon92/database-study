...
-- 정답:
SELECT name AS 이름, date_of_birth AS 생년월일, address AS 주소
FROM members m
...
-- 정답:
SELECT name AS 회원명, title AS 도서명
...
-- 정답:
SELECT title AS 도서명, borrow_date AS 대출날짜
FROM books b
...
SELECT ls.name, role -- 컬럼명이 중복될 시, 테이블명 명시
FROM members m
JOIN library_staff ls ON m.name = ls.name;

SELECT name AS 이름, role AS 역할
FROM library_staff
WHERE members.name = library_staff.name;
