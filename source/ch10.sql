-- Active: 1749606776741@@127.0.0.1@3306@data_modeling
/*
10. 데이터 모델링
10.1 데이터 모델링이란
*/
...
-- 예: 쇼핑몰 사이트
-- 엔티티: 사용자, 주문, 결제, 상품 (나중에 테이블이 될 대상들..)
-- 관계: 주문하다, 결제하다, 장바구니에 담다로 연결
...
CREATE TABLE payments (
  ...
  order_id INTEGER NOT NULL UNIQUE, -- 주문 아이디(필수 입력, 고유키 지정)
  PRIMARY KEY (id), -- 기본키 지정: id
...
CREATE TABLE products (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
  name VARCHAR(50) NOT NULL UNIQUE, -- 상품명(필수 입력, 고유키 지정) <-- 기본키가 될수있는 후보키, 기본키가 있으면 대체키라고부름 (NOT NULL, UNIQUE 조건)
  price INTEGER NOT NULL CHECK (price > 0), -- 가격(필수입력, 양수만 허용) .. UNSIGNED 도 가능
  product_type VARCHAR(50) DEFAULT 'NONE', -- 상품 유형(기본값 NONE) ..명시적으로 null값을 넣으면 null 입력됨
  PRIMARY KEY (id), -- 기본키 지정: id
