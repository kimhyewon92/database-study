...
-- Active: 1749606776741@@127.0.0.1@3306@data_type
/*
9. 서브쿼리 활용하기
9.1 서브쿼리란
*/
...
FROM (
  ...
  SELECT 
    order_id,
    SUM(count) AS total_count -- 집계 함수 결과에 별칭 필수(컬럼명이 아니라 계산한 값을 반환하기 때문에)
  FROM order_details
  GROUP BY order_id
...
SELECT 
  name AS 상품명, 
  total_count AS '주문 개수' -- 더 정확하게 sub.total_count 도 가능
FROM products p
JOIN (
...
-- 4. WHERE 절에서의 서브쿼리
-- 1x1, Nx1 반환하는 서브쿼리만 사용 가능 ..단일값, 다행1열
