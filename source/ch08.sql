-- Active: 1749606776741@@127.0.0.1@3306@market
/*
8. 그룹화 분석하기
8.1 그룹화란
*/
...
-- SELECT 그룹화_컬럼, 집계_함수(일반_컬럼) -- > avg sum count
-- FROM 테이블명
-- WHERE 조건
-- GROUP BY 그룹화_컬럼;
...
-- WHERE AVG(amount) >= 40000 -- 에러.. WHERE 절은 GROUP BY나 집계 함수가 처리되기 전에 실행되기 때문
FROM payments
GROUP BY ptype
...
-- ORDER BY 절을 사용하여 수행
-- SELECT 로 조회된 데이터를 기준으로 정렬하는 작업(SELECT가 먼저 수행됨)
...
SELECT 
  ...
  amount AS '결제 금액'
FROM payments
ORDER BY ptype ASC, amount DESC; -- ptype의 오름참순 정렬 후, amount의 내림차순 정렬(순서 중요)
...
-- 또는 LIMIT M, N; OFFSET 생략시 순서가 바뀜
...
JOIN orders o ON o.user_id = u.id -- 2) 주문 테이블을 붙이고
JOIN payments p ON o.id = p.order_id -- 3) 결제 테이블도 붙이고
GROUP BY nickname -- 4) 사용자별 그룹화
...
SELECT
  name AS '상품명',
  SUM(count) AS '총 판매수량' -- 6)
FROM order_details od -- 1)
JOIN orders o ON o.id = od.order_id -- 2)
JOIN products p ON p.id = od.product_id -- 3)
WHERE status = '배송 완료' -- 4)
GROUP BY name -- 5) ..HAVING 절 있으면 그 후에 SELECT
