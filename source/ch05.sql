-- Active: 1749606776741@@127.0.0.1@3306@store
/*
5. 다양한 자료형 활용하기
5.1 자료형이란
*/
...
CREATE TABLE addresses (
  postal_code CHAR(5), -- 우편번호(고정 길이 문자: 5자), 주로 고정된 길이의 코드값(예: 국가코드 'KR')
  ...
  -- (참고) UTF-8 기준, VARCHAR(65535)는 현실적으로 불가능하고 VARCHAR(16383) 정도가 안전한 최대치 ..한글저장시 3,4로나눌시?
);
...
-- 데이터 베이스에 직접 저장하지 않고, 클라우드 스토리지나 파일 서버 등에 저장한 뒤,
...
-- ④ customer_name이 100자보다 짧으면, 고객명을 저장하고 남은 만큼의 메모리 공간이 절약된다. -> VARCHAR 설명
...
-- YEAR(날짜): 입력 날짜의 '연도' 추출, 예: YEAR('2024-04-15') => 2024 정수값
-- MONTH(날짜): 입력 날짜의 '월' 추출, 예: MONTH('2024-04-15') => 4 정수값
-- DAY(날짜): 입력 날짜의 '일' 추출, 예: DAY('2024-04-15') => 15 정수값
...
SELECT SUM(price * quantity) AS 오전매출
FROM orders
WHERE HOUR(created_at) < 12;
